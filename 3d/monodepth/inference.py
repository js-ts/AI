...
def load_models():
    ...
    dict_encoder = torch.load('./outputs/mono_640x192/encoder.pth', map_location=torch.device('cpu'))
    ...
    # depth decoder
    dict_depth = torch.load('./outputs/mono_640x192/depth.pth', map_location=torch.device('cpu'))
    ...
    dict_poseencoder = torch.load('./outputs/mono_640x192/pose_encoder.pth', map_location=torch.device('cpu'))
    ...
    dict_posedecoder = torch.load('./outputs/mono_640x192/pose.pth', map_location=torch.device('cpu'))
...
if __name__ == '__main__':
    ...
    data, (ow, oh) = load_image('./dataset/test.jpg', size=(640, 192))
    ...
    save_color_depth(disp, 'outputs/test.jpg')
    ...
    disp = F.interpolate(disp, (oh, ow), mode='bilinear', align_corners=False)
    disp = disp.squeeze().cpu().data.numpy()
    save_color_depth(disp, 'outputs/test_v1.jpg')
